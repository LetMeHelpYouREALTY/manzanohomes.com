...
export const PAGES: Record<string, MarketingContent> = {
  ...
  "/privacy-policy": {
    ...
    paragraphs: [
      "Manzano Homes (Berkshire Hathaway HomeServices Nevada Properties) collects name, email, phone, and message content when you submit a form, request a valuation, or ask for a listing alert on ManzanoHomes.com.",
      "We use that data to respond to your inquiry and, when configured, to create or update a Follow Up Boss contact record for Dr. Jan Duffy. RealScout widgets may also collect search filters you enter in the MLS tools. We do not sell personal information.",
      "Pages are served from Vercel. Server logs can include IP address, user agent, and the URL requested. Contact-form posts go to our /api/contact/submit route on this domain.",
      "You can ask us to correct or delete a contact record by emailing the office. We keep inquiry records as long as needed to complete a real-estate transaction or as required by Nevada brokerage record rules.",
      `Questions: ${SITE.email} or ${SITE.phoneDisplay}. Office: ${SITE.address.street}, ${SITE.address.city}, ${SITE.address.region} ${SITE.address.postalCode}. License ${SITE.license}.`,
    ],
    faqs: contactFaq,
    cta: "contact",
  },
  "/terms-of-service": {
    ...
    paragraphs: [
      "ManzanoHomes.com is an advertisement for Dr. Jan Duffy, REALTOR®, Berkshire Hathaway HomeServices Nevada Properties, license S.0197614.LLC. Equal Housing Opportunity.",
      "Listing data is supplied by third-party MLS widgets (RealScout) for consumers' personal, non-commercial use. It may lag the live market. Confirm price, status, square footage, HOA dues, and taxes before you offer.",
      "Home-value widgets and ZIP medians on this site are estimates, not appraisals. 89121, 89178, 89179, and 89138 figures can print differently than a specific plan on Manzano Peak Ave.",
      "Do not copy or scrape listing photos or remarks from the widgets. The brokerage is not responsible for third-party data errors.",
      `Governing office: ${SITE.address.street}, ${SITE.address.city}, ${SITE.address.region} ${SITE.address.postalCode}. ${SITE.phoneDisplay}. ${SITE.email}.`,
    ],
    faqs: contactFaq,
    cta: "contact",
  },
  "/disclaimer": {
    ...
    paragraphs: [
      "RealScout and MLS content on ManzanoHomes.com is for consumers' personal, non-commercial use. It may not be copied or redistributed.",
      "Price, status, square footage, HOA dues, and tax figures can change without notice. Dr. Jan Duffy / BHHS Nevada Properties is not responsible for third-party data errors.",
      "ZIP medians and builder starting prices (including Vales at Manzano from about $315,990) are not a CMA for a specific address on Manzano Peak Ave.",
      "Equal Housing Opportunity. This is not a solicitation if your property is currently listed with another broker.",
    ],
    faqs: contactFaq,
    cta: "contact",
  },
